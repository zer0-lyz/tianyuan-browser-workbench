...
assert.equal(sidepanel.includes("saveWorkBuddyBinding"), true);
assert.equal(sidepanel.includes("clearWorkBuddyBinding"), true);
assert.equal(bridge.includes('req.method === "DELETE"'), true);
assert.equal(installer.includes('"codex_catalog.js"'), true);
assert.equal(codexCatalog.includes("function readCodexCatalog"), true);
...
assert.equal(conversationService.includes("CONVERSATION_NOT_BOUND"), true);
