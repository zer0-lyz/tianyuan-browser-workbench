...
- Windows `0.14.2` 侧栏可发现新版本、下载和校验，但完整更新报 `UPDATE_INSTALLER_NOT_FOUND`。
- 根因是 Windows 发布包从 `0.13.1` 起将入口改为 ASCII `install.ps1`，而 `native-helper/update_installer.js` 仍硬编码查找和启动旧中文 `安装.ps1`。
- macOS 一直使用 `安装.command`，因此不受影响。
- 新更新器优先查找 `install.ps1`，同时兼容历史 `安装.ps1` 包。
- 仅修复新代码不足以让已安装旧更新器自举，因此 Windows 新包同时生成 UTF-8 BOM 的 `安装.ps1` 兼容别名；人工入口仍固定为 ASCII `install.cmd`。
- 新增 Windows 更新器安装测试，确认 ASCII 顶层目录和 `install.ps1` 被正确定位和启动。
- 新增旧版包兼容测试，确认仅含 `安装.ps1` 时仍可完成完整性检查。
- `v0.14.6` 已发布但 Windows 侧栏更新仍受此旧路径问题阻断，由 `v0.14.7` 立即取代。
- 源码提交和标签为 `6191d4440d4920d53352bd50d7d9a5f6c3e0d231` / `v0.14.7`。
...
- 正式 Release 已上线，5 个资产全部上传；在线检查确认 Windows `0.14.2` 可发现 `0.14.7`。
- 正式 ZIP 已同时通过旧更新器中文路径发现和新更新器 ASCII 路径优先验证。
