...
const codexCatalog = fs.readFileSync(path.join(repoRoot, "native-helper", "codex_catalog.js"), "utf8");
...
assert.equal(html.includes('id="clearAgentBinding"'), true);
assert.equal(sidepanel.includes("async function saveWorkBuddyBinding"), true);
assert.equal(sidepanel.includes("async function clearWorkBuddyBinding"), true);
assert.equal(bridge.includes('req.method === "DELETE"') && bridge.includes('parts[3] === "agent-bindings"'), true);
...
assert.equal(installer.includes('copyFileAtomic(path.join(repoRoot, "native-helper", "codex_catalog.js")'), true);
assert.equal(bridge.includes("readCodexCatalog"), true);
assert.equal(bridge.includes("filterRetainedCatalog"), true);
assert.equal(bridge.includes("connector-suite-bridge"), true);
assert.equal(bridge.includes("/api/projects"), true);
assert.equal(nativeHost.includes("codex_catalog.js"), true);
assert.equal(nativeHost.includes("connector-suite-bridge"), true);
assert.equal(codexCatalog.includes("office-connector-registry"), true);
assert.equal(codexCatalog.includes("connector-suite-registry"), true);
assert.equal(codexCatalog.includes("codex-sidebar-catalog"), true);
assert.equal(codexCatalog.includes("catalogOrder"), true);
assert.equal(codexCatalog.includes("archived"), true);
assert.equal(codexCatalog.includes("codex-sqlite-catalog"), true);
assert.equal(codexCatalog.includes("content"), false);
