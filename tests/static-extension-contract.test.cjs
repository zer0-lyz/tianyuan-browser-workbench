...
const ids = [html, updatesTemplate]
...
const referencedIds = [sidepanel, updatesModule, moduleRegistry]
  .flatMap((source) =>
...
assert.equal(versionConfig.productVersion, "0.11.0");
...
assert.equal(html.includes('<span id="moduleCountBadge" class="badge">9 个模块</span>'), true);
