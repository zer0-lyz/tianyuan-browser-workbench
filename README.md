...
“明细表导出与整理”复用旧的“导出明细表”入口：可在导出后按顺序恢复公式、设置明细表格式，也可选择本机工作簿手动整理。自动模式直接提交新导出的文件；手动模式支持覆盖、同目录副本和新目录输出。每个文件先在临时副本中完成全部步骤并校验，失败不会替换原文件。
...
侧栏还提供“浙江土地市场网”“阿里司法拍卖”和“安居客数据”模块。浙江土地市场网每次成功运行会保存历史抓取清单，可从输出目录加载旧结果或按历史记录重新抓取详情。安居客模块的使用流程是：在当前浏览器标签页的安居客网页中选好范围，点击“导入当前网址”，再点击“开始抓取”；抓取不新开浏览器窗口，支持出售/租赁/自动识别、数量上限、验证等待、当前页证据快照、CSV/JSON 和按 A-W 规则生成的 Excel；输出目录会在本机创建“安居客物业案例”子目录，网页未提供的字段保留为空。
